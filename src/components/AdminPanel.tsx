...
import { Trash2, Plus, Edit3, Image as ImageIcon, X } from 'lucide-react';
...
const ImageUploader = ({ onUpload, currentUrl }: { onUpload: (url: string) => void, currentUrl: string }) => {
  ...
  return (
    <div className="space-y-2">
      <input type="file" onChange={handleFileChange} className="block w-full text-xs text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-[#2563EB] file:text-white hover:file:bg-[#1D4ED8]" />
      {currentUrl && (
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">Full editing and deletion capabilities for all academy pages and sections</p>
          </div>
          <button 
            onClick={onClose} 
            ...
            <div className="flex items-center justify-between">
              ...
              <button 
                onClick={() => {
                  ...
                  const newId = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || Date.now().toString();
                ...
                <div key={page.id} className="p-6 bg-[#E0E5EC] rounded-2xl space-y-4 relative border border-gray-200">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#D1D5DB] pb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button 
                        onClick={() => {
                          if (window.confirm(`Delete page "${page.name}" and all its sections?`)) {
                            setPages(pages.filter((_, i) => i !== pIdx));
                          }
                        }} 
                        className="px-3 py-1.5 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white text-xs font-bold rounded-xl flex items-center gap-1 transition-colors"
                      >
                ...
                {languages.map((lang, idx) => (
                  <div key={lang.id} className="p-4 bg-[#E0E5EC] rounded-2xl space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button 
                        onClick={() => {
                          if (window.confirm(`Delete language ${lang.name}?`)) {
                            setLanguages(languages.filter((_, i) => i !== idx));
                          }
                        }} 
                        className="p-2 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-colors"
                      ...
                      <button 
                        onClick={() => {
                          if (window.confirm(`Delete module ${m.title}?`)) {
                            setTrainingModules(trainingModules.filter((_, i) => i !== idx));
                          }
                        }} 
                        className="p-2 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-colors"
                      >
                ...
                {slogans.map((item, idx) => (
                  <div key={item.id} className="p-4 bg-[#E0E5EC] rounded-2xl space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button 
                        onClick={() => {
                          if (window.confirm(`Delete slogan "${item.quote}"?`)) {
                            setSlogans(slogans.filter((_, i) => i !== idx));
                          }
                        }} 
                        className="p-2 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-colors"
                      >
                ...
                {pillars.map((pillar, idx) => (
                  <div key={pillar.id} className="p-4 bg-[#E0E5EC] rounded-2xl space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <button 
                        onClick={() => {
                          if (window.confirm(`Delete pillar "${pillar.title}"?`)) {
            ...
            {homeSections.map((section, index) => (
              <div key={section.id} className="p-6 bg-[#E0E5EC] rounded-2xl space-y-3 relative">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (window.confirm('Delete this home section?')) {
                        setHomeSections(homeSections.filter((_, i) => i !== index));
                      }
                    }}
                    className="p-3 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-colors"
                    title="Delete section"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
              ...
              <div key={item.id} className="p-4 bg-[#E0E5EC] rounded-2xl space-y-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="p-2.5 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-colors"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
                  ...
                  <button
                    onClick={() => {
                      if (window.confirm(`Delete department "${dept.name}"?`)) {
                        setAcademicDepartments(academicDepartments.filter((_, i) => i !== index));
                      }
                    }}
                    className="p-2.5 bg-red-100 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-colors"
                  >
